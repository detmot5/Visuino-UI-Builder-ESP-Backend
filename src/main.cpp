...
namespace WebsiteServer {
  ...
  const char* CORS_HEADER_ACCESS_CONTROL_ALLOW_ORIGIN PROGMEM =
          "Access-Control-Allow-Origin";
  const char* CORS_HEADER_ACCESS_CONTROL_ALLOW_METHODS PROGMEM =
          "Access-Control-Allow-Methods";
  const char* CORS_ALLOWED_METHODS PROGMEM = "POST,GET,OPTIONS";

  const char* CORS_HEADER_ACCESS_CONTROL_ALLOW_HEADERS PROGMEM =
          "Access-Control-Allow-Headers";
  const char* CORS_ALLOWED_HEADERS PROGMEM =
  ...
  namespace ComponentType {
    namespace Input {
      ...
      const char* TextInput PROGMEM = "textInput";
...
// DO NOT MODIFY!
  ...
  namespace ErrorMessage {
    namespace Memory {
      const char* HeapFragmentationTooHigh PROGMEM =
              "Memory - Heap Fragmentation too high, stability problems may occur";
      const char* LowHeapSpace PROGMEM =
              "Memory - Low Heap Space, stability problems may occur";
    } // namespace Memory
    ...
    namespace JsonInput {
      ...
      const char* ElementsArrayEmpty PROGMEM =
              "Json Input - array of elements is empty";
  ...
  namespace Exceptions {
    class RequiredParameterNotFoundException : public std::exception {
    public:
      RequiredParameterNotFoundException(const char* componentName, const char* message) :
    ...
    public:
      InvalidComponentTypeException(const char* componentName, const char* message)
              : componentName(componentName), message(message) {}

      const char* getComponentName() const noexcept {
        return componentName;
      }
...
#if WEBSITE_USE_PSRAM
  using WebsiteJsonDocument = BasicJsonDocument<PSramAllocator>;
#else
  using WebsiteJsonDocument = DynamicJsonDocument;
#endif


// Abstraction on Json Document which is common for few parts of app to make it
// thread safe
  ...
  public:
    ...
    bool allocate(size_t size) {
      mem = new WebsiteJsonDocument(size);
      if (mem->capacity() != 0) {
    ...
    WebsiteJsonDocument * get() { return this->mem; }

  private:
    WebsiteJsonDocument * mem;
    bool m_isLocked;
  ...
  namespace Website {
    ...
    public:
      ...
      // ** THIS METHOD USES COMMON MEMORY(DOCUMENT) FOR STORING JSON TO AVOID
      // MULTIPLE HEAP ALLOCATIONS **
      // ** WHEN YOU GET OBJECT, THE PREVIOUS ONE IS DELETED AUTOMATICALLY **
      // ** REMEMBER TO CHECK THAT MEMORY IS NOT LOCKED ADN WRAP THIS METHOD IN
      // lock() and release() functions to make it thread safe (ESP32)
      ...
      static void setComponentJsonWeakRef(CommonJsonMemory* ref) {
        componentJsonWeakRef = ref;
      }

      static CommonJsonMemory* getComponentJsonWeakRef() {
        return componentJsonWeakRef;
      };

    protected:
      static CommonJsonMemory
              * componentJsonWeakRef; // weak reference to json document stored in
    ...
    WebsiteComponent::WebsiteComponent(const JsonObjectConst& inputObject) {
      if (inputObject.containsKey(JsonKey::Name)) {
        ...
        throw Exceptions::RequiredParameterNotFoundException(this->name.c_str(),
...
// ----------------------------------------------------------------------------
    ...
    public:
      ...
      static VisuinoOutputFormatter& getVisuinoOutputFormatter() {
        return formatter;
      }
    ...
    public:
      explicit Slider(const JsonObjectConst& inputObject)
        ...
        if (inputObject.containsKey(JsonKey::Height)) {
          ...
          this->height = DefaultValues::SliderHeight;
    ...
    public:
      explicit TextInput(const JsonObjectConst& inputObject)
              : InputComponent(inputObject) {
        if (inputObject.containsKey(JsonKey::Width)) {
          this->width = inputObject[JsonKey::Width];
        } else {
          throw Exceptions::RequiredParameterNotFoundException(this->name.c_str(),
                                                               "TextInput width not found!");
        }
        if (inputObject.containsKey(JsonKey::Height)) {
          this->height = inputObject[JsonKey::Height];
        } else {
          throw Exceptions::RequiredParameterNotFoundException(this->name.c_str(),
                                                               "TextInput height not found!");
        }
        if (inputObject.containsKey(JsonKey::FontSize)) {
          this->fontSize = inputObject[JsonKey::FontSize];
        } else {
      ...
      JsonObject toWebsiteJson() override {
        ...
        websiteObj[JsonKey::FontSize] = this->fontSize;
        websiteObj[JsonKey::Color] = this->color;
        websiteObj[JsonKey::ComponentType] = ComponentType::Input::TextInput;
        return websiteObj;
      }
    ...
    VisuinoOutputFormatter TextInput::formatter;
    ...
    public:
      explicit Button(const JsonObjectConst& inputObject)
        ...
        if (inputObject.containsKey(JsonKey::Width)) {
          ...
          throw Exceptions::RequiredParameterNotFoundException(this->name.c_str(),
                                                               "Button width not passed!");
        }
        if (inputObject.containsKey(JsonKey::Height)) {
          this->height = inputObject[JsonKey::Height];
        } else {
          throw Exceptions::RequiredParameterNotFoundException(this->name.c_str(),
                                                               "Button height not passed!");
        }
        if (inputObject.containsKey(JsonKey::FontSize)) {
          this->fontSize = inputObject[JsonKey::FontSize];
        } else
        ...
        if (inputObject.containsKey(JsonKey::IsVertical)) {
          ...
          this->isVertical = DefaultValues::IsVertical;
        this->value = false;
      }
    ...
    public:
      explicit LedIndicator(const JsonObjectConst& inputObject)
        ...
        if (inputObject.containsKey(JsonKey::Color)) {
          ...
          this->color = DefaultValues::LedColor;
      }
    ...
    public:
      explicit ProgressBar(const JsonObjectConst& inputObject)
        ...
        if (inputObject.containsKey(JsonKey::Color)) {
          ...
          this->color = DefaultValues::Color2;
        if (inputObject.containsKey(JsonKey::Width)) {
          this->width = inputObject[JsonKey::Width];
        } else
          this->width = DefaultValues::Width;
        if (inputObject.containsKey(JsonKey::Height)) {
          ...
          this->height = DefaultValues::BarHeight;
        if (inputObject.containsKey(JsonKey::IsVertical)) {
          this->isVertical = inputObject[JsonKey::IsVertical];
        } else
          this->isVertical = DefaultValues::IsVertical;
    ...
    public:
      explicit ColorField(const JsonObjectConst& inputObject)
        ...
        if (inputObject.containsKey(JsonKey::FieldOutlineColor)) {
          this->outlineColor =
                  inputObject[JsonKey::FieldOutlineColor].as<const char*>();
        } else
          this->outlineColor = DefaultValues::FieldOutlineColor;
      }
    ...
    public:
      explicit BackgroundImage(const JsonObjectConst& inputObject)
        ...
        if (inputObject.containsKey(JsonKey::FileName)) {
          ...
          throw Exceptions::RequiredParameterNotFoundException(this->name.c_str(),
                                                               "Filename in BackgroundImage is "
                                                               "not specified!");
        }
        if (inputObject.containsKey(JsonKey::Width)) {
          this->width = inputObject[JsonKey::Width];
    ...
    public:
      ...
      bool onComponentInputHttpRequest(CommonJsonMemory* target,
    ...
    WebsiteTab::WebsiteTab(const char* name, const JsonArrayConst componentsJson)
            : name(name) {
    ...
    void WebsiteTab::onUpdate(const JsonArrayConst componentsJson) {
      ...
      for (const auto component: componentsJson) {
        ...
        } catch (Exceptions::InvalidComponentTypeException& e) {
          Log::info(e.getComponentName());
          Log::error(e.what());
        }
      }
    }
    ...
    WebsiteTab::ComponentParsingStatus
    WebsiteTab::appendComponent(const JsonObjectConst& object) {
      if (!object.containsKey(JsonKey::Name) ||
          !object.containsKey(JsonKey::ComponentType))
        return ComponentParsingStatus::OBJECT_NOT_VALID;
      const char* name = object[JsonKey::Name];
      const char* componentType = object[JsonKey::ComponentType];
      if (strlen(componentType) < 1) {
        throw Exceptions::InvalidComponentTypeException(name, "Component type not found!");
        return ComponentParsingStatus::COMPONENT_TYPE_NOT_FOUND;
      }
      ...
      if (!strncmp(componentType, Input::Switch, strlen(componentType))) {
      ...
      } else if (!strncmp(componentType, Input::NumberInput,
                          strlen(componentType))) {
      ...
      } else if (!strncmp(componentType, Input::TextInput, strlen(componentType))) {
        parseInputComponentToWebsite<TextInput>(object);
      ...
      } else if (!strncmp(componentType, Output::Indicator,
                          strlen(componentType))) {
        parseOutputComponentToWebsite<LedIndicator>(object);
      } else if (!strncmp(componentType, Output::ProgressBar,
                          strlen(componentType))) {
        ...
        throw Exceptions::InvalidComponentTypeException(name, "Invalid component type!");
      }
    ...
    bool WebsiteTab::parseInputComponentToWebsite(const JsonObjectConst& object) {
      ...
      if (!componentAlreadyExists(componentName)) {
        ...
        try {
          component = new ComponentType(object);
          components.push_back(component);

        } catch (Exceptions::RequiredParameterNotFoundException& e) {
          Log::info(e.getComponentName());
          Log::error(e.what());
          delete component;
          return false;
    ...
    bool WebsiteTab::parseOutputComponentToWebsite(const JsonObjectConst& object) {
      ...
      if (!componentAlreadyExists(componentName)) {
        ...
        auto component =
                reinterpret_cast<ComponentType*>(getComponentByName(componentName));
    ...
    bool WebsiteTab::parseInputComponentToVisuino(const JsonObjectConst& object) {
      const char* componentName = object[JsonKey::Name];
      auto component =
              reinterpret_cast<InputComponent*>(getComponentByName(componentName));
      if (component == nullptr)
        return false;
      component->setState(object);
    ...
    bool WebsiteTab::componentAlreadyExists(const char* componentName) {
      bool res = false;
      if (getComponentByName(componentName) != nullptr)
        res = true;
      return res;
    }

    bool WebsiteTab::onComponentInputHttpRequest(CommonJsonMemory* target,
                                                 const uint8_t* data, size_t len) {
      ...
      if (!strncmp(componentType, Input::Switch, strlen(componentType))) {
        ...
        return parseInputComponentToVisuino<TextInput>(receivedJson);
      }
      return false;
    }
  ...
  public:
    ...
    static inline CommonJsonMemory* getStateJsonWeakRef() {
      return &stateJsonMemory;
    }

    static inline CommonJsonMemory* getComponentJsonWeakRef() {
      return &componentJsonMemory;
    }

    static inline CommonJsonMemory* getVisuinoOutputJsonWeakRef() {
      return &visuinoOutputJsonMemory;
    }
  ...
  bool ApplicationContext::updateFlag = false;
  ...
  void ApplicationContext::parseTabs(const JsonObjectConst& tabsJson) {
    ...
    for (JsonPairConst tabJson: tabsJson) {
      ...
      if (tab == nullptr) {
        tabs.push_back(new Website::WebsiteTab(
                tabName, tabJson.value().as<JsonArrayConst>()));
      } else {
  ...
  bool ApplicationContext::onComponentStatusHTTPRequest(const uint8_t* data,
                                                        size_t len) {
    for (auto& tab: tabs) {
  ...
  namespace JsonReader {
    ...
    size_t getBiggestObjectSize(const JsonObjectConst& arr) {
      ...
      for (const JsonPairConst item: arr) {
        if (item.value().memoryUsage() > biggest)
          biggest = item.value().memoryUsage();
      }
      return biggest;
    }
...
// ESP32 needs second JSON document because of multicore architecture and it
// could not be common with input memory
    bool allocateVisuinoOutputMemory(size_t size) {
      auto visuinoOutputJsonWeakRef =
              ApplicationContext::getVisuinoOutputJsonWeakRef();
#ifdef ESP32
...
#endif
#ifdef ESP8266
      // ESP8266 is not multithreaded so visuino output can use component output
      // memory It saves few bytes. Anyway, this tool is recommended to work with
      // ESP32 to reach better reliability
      visuinoOutputJsonWeakRef = ApplicationContext::getComponentJsonWeakRef();
      Website::WebsiteComponent::setVisuinoOutputJsonWeakRef(
          visuinoOutputJsonWeakRef);
      return true;
#endif
    }
    ...
    InputJsonStatus readWebsiteComponentsFromJson(const String& json) {
      ...
      if (isValid) {
        ...
        if (stateJsonWeakRef->isReadyToUse()) {
          ...
          if (stateJsonWeakRef->get()->overflowed())
            return releaseAndReturn(InputJsonStatus::JSON_OVERFLOW);
          ...
          if (!isElementsInitialized) {
            ...
            if (!allocateVisuinoOutputMemory(getBufferSize(biggestObjectSize)))
              return releaseAndReturn(InputJsonStatus::ALLOC_ERROR);
            auto componentJsonWeakRef =
                    ApplicationContext::getComponentJsonWeakRef();
  ...
  namespace JsonWriter {
    void write() {
      ...
      static auto& textInputFormatter = TextInput::getVisuinoOutputFormatter();
      ...
      if (textInputFormatter.isDataReady) {
        Serial.println(textInputFormatter.getData());
        textInputFormatter.isDataReady = false;
      }
    }
  } // namespace JsonWriter
  ...
  void fullCorsAllow(AsyncWebServerResponse* response) {
    ...
    response->addHeader(CORS_HEADER_ACCESS_CONTROL_ALLOW_METHODS,
                        CORS_ALLOWED_METHODS);
    response->addHeader(CORS_HEADER_ACCESS_CONTROL_ALLOW_HEADERS,
  ...
  void HTTPSetMappings(AsyncWebServer& webServer) {
    ...
    webServer.on("/state", HTTP_GET, [](AsyncWebServerRequest* request) {
      using namespace Website;
      static String responseBody; // static to avoid heap allocation in every
      // request - beginResponse takes const reference
      auto stateJsonWeakRef = ApplicationContext::getStateJsonWeakRef();
#ifdef DEBUG_BUILD
      ...
      if (stateJsonWeakRef->isReadyToUse()) {
        ...
        AsyncWebServerResponse* response = request->beginResponse(
                HttpCodes::OK, "application/json", responseBody);
    ...
    webServer.on(
            "/input", HTTP_POST, [](AsyncWebServerRequest* request) {}, nullptr,
      ...
      if (request->hasParam("fileName")) {
        ...
        if (SPIFFS.exists(fileName)) {
          ...
          if (f.size() > IMAGE_MAX_SIZE) {
            ...
            Log::error(
                    "Requested file is too large or system has not enough memory",
                    Serial);
            f.close();
          } else {
            f.close();
            AsyncWebServerResponse* response =
                    request->beginResponse(SPIFFS, fileName, "image/jpg");
  ...
  void registerWiFIDebugEvents() {
    WiFi.onEvent([](WiFiEvent_t event,
                    WiFiEventInfo_t info) { Log::info("AP connected!"); },
                 SYSTEM_EVENT_AP_STACONNECTED);

    WiFi.onEvent([](WiFiEvent_t event,
                    WiFiEventInfo_t info) { Log::info("AP_Disconnected!"); },
                 SYSTEM_EVENT_AP_STADISCONNECTED);

    WiFi.onEvent([](WiFiEvent_t event,
                    WiFiEventInfo_t info) { Log::info("STA_Connected"); },
                 SYSTEM_EVENT_STA_CONNECTED);
    WiFi.onEvent([](WiFiEvent_t event,
                    WiFiEventInfo_t info) { Log::info("STA_Disconnected"); },
...
void debugTask(void* args) {
  esp_task_wdt_add(nullptr);
  for (;;) {
    if (WebsiteServer::ApplicationContext::updateFlag) {
      WebsiteServer::JsonReader::readWebsiteComponentsFromJson(testWebsiteConfigStr);
      WebsiteServer::ApplicationContext::updateFlag = true;
    }
    delay(100);
    esp_task_wdt_reset();
  }
...
void setup() {
  ...
  WiFi.begin("UPC1452532", "Rckssta7mmK7");
  ...
  xTaskCreatePinnedToCore(debugTask, "task", 4096, nullptr, 1, nullptr, 0);
}
...
void loop() {
  ...
  if (actual - previous > interval) {
    ...
    using namespace WebsiteServer;
    using namespace WebsiteServer::JsonReader;
    InputJsonStatus status =
            readWebsiteComponentsFromJson(testWebsiteConfigStr);
    uint32_t after = millis();
    previous = actual;
  }
